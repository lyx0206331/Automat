...
public final class LocalBroadcastManager {
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                CommUtil.logV("LocalBroadcastManager", "Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            }

            ArrayList entries = (ArrayList) this.mActions.get(intent.getAction());
            if (entries != null) {
                if (debug) {
                    CommUtil.logV("LocalBroadcastManager", "Action list: " + entries);
                }
                ...
                for (i = 0; i < entries.size(); ++i) {
                    ...
                    if (debug) {
                        CommUtil.logV("LocalBroadcastManager", "Matching against filter " + receiver.filter);
                    }

                    if (receiver.broadcasting) {
                        if (debug) {
                            CommUtil.logV("LocalBroadcastManager", "  Filter\'s target already added");
                        }
                    } else {
                        int match = receiver.filter.match(action, type, scheme, data, categories, "LocalBroadcastManager");
                        if (match >= 0) {
                            if (debug) {
                                CommUtil.logV("LocalBroadcastManager", "  Filter matched!  match=0x" + Integer.toHexString(match));
                            }
                            ...
                            CommUtil.logV("LocalBroadcastManager", "  Filter did not match: " + reason);
                        }
                    }
                }
